...
export const Sidebar: React.FC<SidebarProps> = ({ currentTab, onTabSelect }) => {
  ...
  const sections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'history', label: 'Historique Trajets', icon: History },
      ],
    },
    {
      title: 'ANALYTICS',
  ...
  return (
    <aside
      ...
      } bg-slate-900/95 dark:bg-slate-900/95 light:bg-slate-900 border-r border-slate-800/80 flex flex-col h-screen shrink-0 select-none transition-all duration-300 relative z-20`}
    >
      {/* Brand Header */}
        ...
        <div className="flex items-center space-x-3 overflow-hidden">
          ...
          {!isCollapsed && (
            <div className="overflow-hidden">
              <h1 className="text-base font-black text-white tracking-wider font-mono leading-tight">
        ...
        {sections.map((section) => {
          ...
          return (
            <div key={section.title} className="space-y-1">
              {!isCollapsed && (
                <div className="px-3 pb-1 text-[10px] font-bold text-slate-500 tracking-wider uppercase">
                  {section.title}
                </div>
              )}
      ...
      <div className="p-3 border-t border-slate-800/80 bg-slate-950/40 text-center">
        {!isCollapsed ? (
          <div>
            ...
            <div className="text-[9px] text-slate-500 mt-0.5">v1.2.0 • SaaS Production</div>
          </div>
        ) : (
