...
export const LiveTrackingPage: React.FC<LiveTrackingPageProps> = ({
  ...
  const { traccarConnected, wsStatus, lastSyncTime, reconnect } = useTraccar();
  ...
  // Filter vehicles by search query & status chip
  ...
  return (
    <div className="h-[calc(100vh-6.5rem)] flex flex-col space-y-3 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 glass-panel rounded-xl border border-slate-800 shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs font-mono">
            {wsStatus === 'CONNECTED' ? (
              <span className="flex items-center gap-1.5 text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded-lg border border-emerald-500/30">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                TRACCAR 6.5 CONNECTÉ
              </span>
            ) : wsStatus === 'RECONNECTING' || wsStatus === 'CONNECTING' ? (
              <span className="flex items-center gap-1.5 text-amber-400 font-bold bg-amber-500/10 px-2 py-0.5 rounded-lg border border-amber-500/30">
              ...
              <button
                onClick={reconnect}
                className="flex items-center gap-1.5 text-rose-400 font-bold bg-rose-500/10 hover:bg-rose-500/20 px-2 py-0.5 rounded-lg border border-rose-500/30 transition-colors"
                title="Cliquer pour forcer la reconnexion Traccar"
              >
                <span className="w-2 h-2 rounded-full bg-rose-500" />
                TRACCAR DÉCONNECTÉ (Réessayer)
              </button>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 flex gap-3 min-h-0 relative">
        {/* Left Fleet Panel (Traccar-Grade Roster) */}
        <div
          ...
          {!isSidebarCollapsed && (
            <>
              ...
              <div className="relative mb-2">
                ...
                <input
                  type="text"
                  placeholder="Rechercher nom, plaque, IMEI..."
                  value={searchQuery}
              ...
              <div className="flex-1 overflow-y-auto space-y-1.5 pr-0.5">
                {filteredVehicles.length === 0 ? (
                  <div className="text-center py-8 text-slate-500 text-xs">
                    {vehicles.length === 0 ? 'Aucun traceur enregistré.' : 'Aucun véhicule correspondant.'}
                  </div>
                ) : (
                  filteredVehicles.map((v) => {
                    ...
                    const lastPosTimeStr = v.last_position_time
                      ? new Date(v.last_position_time).toLocaleTimeString('fr-FR', {
                          hour: '2-digit',
                          minute: '2-digit',
                          second: '2-digit',
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-slate-800/80 text-[10px] text-slate-400 font-mono">
                          ...
                          <span className="text-slate-400">{lastPosTimeStr}</span>
                        </div>
                      </div>
                    );
                  })
        ...
        <div className="flex-1 h-full min-w-0 relative">
          ...
          {currentSelectedVehicle && (
            <div className="absolute bottom-4 left-4 z-20 glass-card p-3.5 rounded-xl border border-slate-700/90 shadow-2xl max-w-sm hidden md:block backdrop-blur-xl pointer-events-auto">
              <div className="flex items-center justify-between gap-3 border-b border-slate-800 pb-2 mb-2">
                <div>
                  ...
                  <p className="text-[10px] text-cyan-400 font-mono font-bold">
                    {currentSelectedVehicle.plate_number} • {currentSelectedVehicle.brand} {currentSelectedVehicle.model}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-slate-300 mb-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-cyan-300 font-bold truncate max-w-[90px]">
                    {currentSelectedVehicle.driver_name || 'N/D'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Batterie:</span>
                  <span className="text-emerald-400 font-bold">
                    {currentSelectedVehicle.battery_level !== null && currentSelectedVehicle.battery_level !== undefined
                      ? `${currentSelectedVehicle.battery_level}%`
                      : 'N/D'}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span>
                    {currentSelectedVehicle.odometer_km ? `${currentSelectedVehicle.odometer_km.toLocaleString('fr-FR')} km` : 'N/D'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Contact:</span>
                  <span className={currentSelectedVehicle.ignition_on ? 'text-emerald-400' : 'text-slate-400'}>
                    {currentSelectedVehicle.ignition_on ? 'ON' : 'OFF'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title="Coupe-moteur / Relais télématique"
                >
